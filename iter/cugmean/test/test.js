...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof itercugmean, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an iterator protocol-compliant object which iteratively computes a cumulative geometric mean', function test( t ) {
	var expected;
	var actual;
	var values;
	var acc;
	...
	it = itercugmean( array2iterator( values ) );
...
tape( 'if an iterated value is a non-numeric value, the computed geometric mean is `NaN`', function test( t ) {
	...
	expected = [ acc(1.0), acc(2.0), NaN, NaN ];
...
tape( 'if an environment supports `Symbol.iterator` and the provided iterator is iterable, the returned iterator is iterable', function test( t ) {
	...
	itercugmean = proxyquire( './../lib/main.js', {
