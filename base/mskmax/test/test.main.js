...
tape( 'the function has an arity of 5', function test( t ) {
	t.strictEqual( mskmax.length, 5, 'has expected arity' );
	t.end();
});
...
tape( 'the function calculates the maximum value of a strided array according to a mask (accessor)', function test( t ) {
	var mask;
	var x;
	var v;
	...
	x = [ 1.0, -2.0, -4.0, NaN, 5.0, 0.0, 3.0 ];
	mask = [ 0, 0, 0, 1, 0, 0, 0 ];
	v = mskmax( x.length, toAccessorArray( x ), 1, toAccessorArray( mask ), 1 );
	t.strictEqual( v, 5.0, 'returns expected value' );
...
tape( 'the function supports `stride` parameters (accessor)', function test( t ) {
	...
	v = mskmax( 5, toAccessorArray( x ), 2, toAccessorArray( mask ), 2 );
...
tape( 'the function supports view offsets (accessor)', function test( t ) {
	...
	v = mskmax( 5, toAccessorArray( x1 ), 2, toAccessorArray( mask1 ), 2 );
	t.strictEqual( v, 4.0, 'returns expected value' );
