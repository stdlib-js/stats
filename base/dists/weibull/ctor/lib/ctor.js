...
function Weibull() {
	...
	if ( arguments.length ) {
		...
		lambda = arguments[ 1 ];
		if ( !isPositive( k ) ) {
			throw new TypeError( format( 'invalid argument. Shape parameter `k` must be a positive number. Value: `%s`.', k ) );
		}
		if ( !isPositive( lambda ) ) {
			throw new TypeError( format( 'invalid argument. Scale parameter `lambda` must be a positive number. Value: `%s`.', lambda ) );
		}
	} else {
		k = 1.0;
		lambda = 1.0;
