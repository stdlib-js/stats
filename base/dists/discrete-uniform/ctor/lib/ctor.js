...
function DiscreteUniform() {
	...
	if ( arguments.length ) {
		...
		if ( a > b ) {
			throw new RangeError( format( 'invalid arguments. Minimum support `a` must be less than or equal to maximum support `b`. Value: `[%d,%d]`.', a, b ) );
		}
	} else {
		a = 0;
		b = 1;
	...
	defineProperty( this, 'a', {
		...
		'set': function set( value ) {
			...
			if ( value > b ) {
				throw new RangeError( format( 'invalid value. Must be less than or equal to `b`. Value: `%d`.', value ) );
			}
			a = value;
		}
	});
