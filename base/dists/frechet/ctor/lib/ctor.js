...
function Frechet() {
	...
	if ( arguments.length ) {
		...
		if ( !isPositive( s ) ) {
			throw new TypeError( format( 'invalid argument. Scale parameter `s` must be a number primitive. Value: `%s`.', s ) );
		}
		if ( !isNumber( m ) || isnan( m ) ) {
			throw new TypeError( format( 'invalid argument. Location parameter `m` must be a number primitive. Value: `%s`.', m ) );
		}
	} else {
		alpha = 1.0;
		s = 1.0;
