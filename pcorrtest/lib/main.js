...
* @throws {Error} first and second arguments must be arrays of the same length
...
function pcorrTest( x, y, options ) {
	...
	n = x.length;
	if ( n !== y.length ) {
		throw new Error( 'invalid arguments. First and second arguments must be arrays of the same length.' );
	}
	opts = {};
	if ( options ) {
	...
	if ( n < 4 ) {
		throw new Error( 'not enough observations. First and second arguments must contain at least four observations.' );
	}
	if ( opts.rho === void 0 ) {
		rho = 0.0;
	} else {
